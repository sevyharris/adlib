...
def setup_kpts_converge(bulk_dir, lattice_constant_center, metal='Cu'):
    """
    ...
    for i, k in enumerate(kpts):
        for j, lattice_constant in enumerate(lattice_constants):
            ...
            adlib.bulk.calc.make_scf_calc_file(calc_dir, lattice_constant, metal=metal, ecutwfc=500, kpt=k, smear=0.1, nproc=16)

        adlib.bulk.calc.make_scf_run_file_array(os.path.join(kpts_dir, str(k)), j, job_name='kpts_bulk_converge')


def setup_ecutwfc_converge(bulk_dir, lattice_constant_center, metal='Cu'):
    """
